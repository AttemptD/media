...
/**
 * A media player interface defining high-level functionality, such as the
 ...
 *
 * <ul>
 * <li>All methods must be called from a single
 * {@linkplain #getApplicationLooper() application
 * thread} unless indicated otherwise. Callbacks in registered listeners are
 * called on the
 * same thread.
 * <li>The available functionality can be limited. Player instances provide a
 * set of {@link
 * #getAvailableCommands() available commands} to signal feature support and
 * users of the
 * interface must only call methods if the corresponding {@link Command} is
 * available. An
 * implementation has some flexibility in how to handle a call to a method when
 * the
 ...
 * <li>Perform some 'default' version of the requested operation (e.g.
 * {@link #seekTo(long)}
 * may trigger {@link #seekToDefaultPosition()} if called when {@link
 * #COMMAND_SEEK_IN_CURRENT_MEDIA_ITEM} is not available).
 * </ul>
 * <li>Users can register {@link Player.Listener} callbacks that get informed
 * about state changes.
 * <li>Player instances need to update the visible state immediately after each
 * method call, even
 * if the actual changes are handled on background threads or even other
 * devices. This
 * simplifies the usage for callers of methods as no asynchronous handling needs
 * to be
 * considered.
 * <li>Player instances can provide playlist operations, like 'set', 'add',
 * 'remove', 'move' or
 * 'replace' of {@link MediaItem} instances. The player can also support
 * {@linkplain
 * RepeatMode repeat modes} and shuffling within this playlist. The player
 * provides a {@link
 * Timeline} representing the structure of the playlist and all its items, which
 * can be
 * obtained by calling {@link #getCurrentTimeline()}
 * <li>Player instances can provide seeking within the currently playing item
 * and to other items,
 * using the various {@code seek...} methods.
 * <li>Player instances can provide {@link Tracks} defining the currently
 * available and selected
 * tracks, which can be obtained by calling {@link #getCurrentTracks()}. Users
 * can also modify
 * track selection behavior by setting {@link TrackSelectionParameters} with
 * {@link
 * #setTrackSelectionParameters}.
 * <li>Player instances can provide {@link MediaMetadata} about the currently
 * playing item, which
 * can be obtained by calling {@link #getMediaMetadata()}.
 * <li>Player instances can provide information about ads in its media
 * structure, for example via
 * {@link #isPlayingAd()}.
 * <li>Player instances can accept different types of video outputs, like {@link
 * #setVideoSurfaceView SurfaceView} or {@link #setVideoTextureView TextureView}
 * for video
 * rendering.
 * <li>Player instances can handle {@linkplain #setPlaybackSpeed playback
 * speed}, {@linkplain
 * #getAudioAttributes audio attributes}, and {@linkplain #setVolume audio
 * volume}.
 * <li>Player instances can provide information about the
 * {@linkplain #getDeviceInfo playback
 ...
 *
 * <p>
 * The majority of the Player interface and its related classes are part of the
 * stable API that
 * guarantees backwards-compatibility for users of the API. Only more advances
 * use cases may need to
 * rely on {@link UnstableApi} classes and methods that are subject to
 * incompatible changes or even
 * removal in a future release. Implementors of the Player interface are not
 * covered by these API
 * stability guarantees.
 *
 * <h2>Player state</h2>
 *
 * <p>
 * Users can listen to state changes by adding a {@link Player.Listener} with
 * {@link
 * #addListener}.
 *
 * <p>
 ...
 * <li>Playlist
 * <ul>
 * <li>{@link MediaItem} instances can be added with methods like
 * {@link #setMediaItem} to
 * define what the player will be playing.
 * <li>The current playlist can be obtained via {@link #getCurrentTimeline} and
 * convenience
 * methods like {@link #getMediaItemCount} or {@link #getCurrentMediaItem}.
 * <li>With an empty playlist, the player can only be in {@link #STATE_IDLE} or
 * {@link
 * #STATE_ENDED}.
 * </ul>
 * <li>Playback state
 * <ul>
 * <li>{@link #STATE_IDLE}: This is the initial state, the state when the player
 * is
 * {@linkplain #stop stopped}, and when playback {@linkplain #getPlayerError
 * failed}.
 * The player will hold only limited resources in this state. {@link #prepare}
 * must be
 * called to transition away from this state.
 * <li>{@link #STATE_BUFFERING}: The player is not able to immediately play from
 * its current
 * position. This mostly happens because more data needs to be loaded.
 * <li>{@link #STATE_READY}: The player is able to immediately play from its
 * current
 * position.
 * <li>{@link #STATE_ENDED}: The player finished playing all media, or there is
 * no media to
 * play.
 * </ul>
 * <li>Play/Pause, playback suppression and isPlaying
 * <ul>
 * <li>{@linkplain #getPlayWhenReady() playWhenReady}: Indicates the user
 * intention to play.
 * It can be set with {@link #play} or {@link #pause}.
 * <li>{@linkplain #getPlaybackSuppressionReason() playback suppression}:
 * Defines a reason
 * for which playback will be suppressed even if {@linkplain #getPlayWhenReady()
 * playWhenReady} is {@code true}.
 * <li>{@link #isPlaying()}: Whether the player is playing (that is, its
 * position is
 * advancing and media is being presented). This will only be {@code true} if
 * playback
 * state is {@link #STATE_READY}, {@linkplain #getPlayWhenReady() playWhenReady}
 * is
 * {@code true}, and playback is not suppressed.
 * </ul>
 * <li>Playback position
 * <ul>
 * <li>{@linkplain #getCurrentMediaItemIndex() media item index}: The index in
 * the playlist.
 * <li>{@linkplain #isPlayingAd() ad insertion}: Whether an inserted ad is
 * playing and which
 ...
 * <li>{@linkplain #getCurrentPosition() current position}: The current position
 * of the
 * playback. This is the same as the {@linkplain #getContentPosition() content
 * position}
 * unless an ad is playing, where this indicates the position in the inserted
 * ad.
 * </ul>
 * </ul>
 *
 * <p>
 * Note that there are no callbacks for normal playback progression, only for
 * {@linkplain
 * Listener#onMediaItemTransition transitions between media items} and other
 * {@linkplain
 * Listener#onPositionDiscontinuity position discontinuities}. Code that needs
 * to monitor playback
 * progress (for example, an UI progress bar) should query the current position
 * in appropriate
 * intervals.
 *
 ...
 *
 * <p>
 * Implementing the Player interface is complex, as the interface includes many
 * convenience
 * methods that need to provide a consistent state and behavior, requires
 * correct handling of
 * listeners and available commands, and expects immediate state changes even if
 * methods are
 * internally handled asynchronously. For this reason, implementations are
 * advised to inherit {@link
 * SimpleBasePlayer} that handles all of these complexities and provides a
...
public interface Player {
  ...
  final class Events {
     ...
     *
     * <p>
     * Although index-based access is possible, it doesn't imply a particular order
     * of these
     * events.
     *
     * @param index The index. Must be between 0 (inclusive) and {@link #size()}
     *              (exclusive).
  ...
  final class PositionInfo {
    ...
    /**
     * The UID of the window, or {@code null} if the timeline is
     * {@link Timeline#isEmpty() empty}.
     */
    @Nullable
    ...
    /**
     * The media item, or {@code null} if the timeline is {@link Timeline#isEmpty()
     * empty}.
     */
    @UnstableApi
    @Nullable
    ...
    /**
     * The UID of the period, or {@code null} if the timeline is
     * {@link Timeline#isEmpty() empty}.
     */
    @Nullable
    ...
    /**
     * The index of the ad within the ad group if the playback position is within an
     * ad, {@link
    ...
    /**
     * @deprecated Use
     *             {@link #PositionInfo(Object, int, MediaItem, Object, int, long, long, int,
     *             int)} instead.
     */
    @Deprecated
    @UnstableApi
    ...
    public String toString() {
      String positionInfoString = "mediaItem=" + mediaItemIndex + ", period=" + periodIndex + ", pos=" + positionMs;
    ...
    /**
     * Returns whether this position info and the other position info would result
     * in the same
    ...
    /**
     * Returns a copy of this position info, filtered by the specified available
     * commands.
     *
     * <p>
     ...
     *
     * @param canAccessCurrentMediaItem Whether
     *                                  {@link Player#COMMAND_GET_CURRENT_MEDIA_ITEM}
     *                                  is
     *                                  available.
     * @param canAccessTimeline         Whether {@link Player#COMMAND_GET_TIMELINE}
     *                                  is available.
     ...
     *
     * <p>
     * It omits the {@link #windowUid} and {@link #periodUid} fields. The
     * {@link #windowUid} and
     * {@link #periodUid} of an instance restored by
     * {@link #fromBundle(Bundle, int)} will always be
     * {@code null}.
     *
     * @param interfaceVersion The {@link MediaLibraryInfo#INTERFACE_VERSION} of the
     *                         receiving
     *                         process.
     */
    @UnstableApi
    ...
    public static PositionInfo fromBundle(Bundle bundle, int interfaceVersion) {
      ...
      int adIndexInAdGroup = bundle.getInt(FIELD_AD_INDEX_IN_AD_GROUP, /* defaultValue= */ C.INDEX_UNSET);
      return new PositionInfo(
          /* windowUid= */ null,
          mediaItemIndex,
          mediaItem,
  ...
  final class Commands {
    ...
    public static final class Builder {
      ...
      private static final FlagSet SUPPORTED_READ_COMMANDS = new FlagSet.Builder()
          .addAll(
      ...
      private static final FlagSet SUPPORTED_WRITE_COMMANDS = new FlagSet.Builder()
          .addAll(
              COMMAND_PLAY_PAUSE,
              COMMAND_PREPARE,
              COMMAND_STOP,
      ...
      /**
       * Adds a {@link Command} if the provided condition is true. Does nothing
       * otherwise.
       *
       * @param command   A {@link Command}.
      ...
      /**
       * Adds all existing {@linkplain Command commands} that provide read access to
       * the {@link
      ...
      /**
       * Removes a {@link Command} if the provided condition is true. Does nothing
       * otherwise.
       *
       * @param command   A {@link Command}.
       * @param condition A condition.
    ...
    /**
     * Returns whether the set of commands contains at least one of the given
     * {@code commands}.
     */
   ...
   *
   * <p>
   * If the return value of a {@link Player} getter changes due to a change in
   * {@linkplain
   * #onAvailableCommandsChanged(Commands) command availability}, the
   * corresponding listener
   * method(s) will be invoked. If the return value of a {@link Player} getter
   * does not change
   * because the corresponding command is
   * {@linkplain #onAvailableCommandsChanged(Commands) not
  ...
  interface Listener {
     ...
     *
     * <p>
     * State changes and events that happen within one {@link Looper} message queue
     * iteration are
     * reported together and only after all individual callbacks were triggered.
     *
     * <p>
     * Listeners should prefer this method over individual callbacks in the
     * following cases:
     *
     * <ul>
     * <li>They intend to trigger the same logic for multiple events (e.g. when
     * updating a UI for
     * both {@link #onPlaybackStateChanged(int)} and
     * {@link #onPlayWhenReadyChanged(boolean,
     * int)}).
     * <li>They need access to the {@link Player} object to trigger further events
     * (e.g. to call
     * {@link Player#seekTo(long)} after a
     * {@link #onMediaItemTransition(MediaItem, int)}).
     * <li>They intend to use multiple state values together or in combination with
     * {@link Player}
     * getter methods. For example using {@link #getCurrentMediaItemIndex()} with
     * the {@code
     *       timeline} provided in {@link #onTimelineChanged(Timeline, int)} is only
     * safe from
     * within this method.
     * <li>They are interested in events that logically happened together (e.g
     * {@link
     * #onPlaybackStateChanged(int)} to {@link #STATE_BUFFERING} because of {@link
     * #onMediaItemTransition(MediaItem, int)}).
     * </ul>
     *
     * @param player The {@link Player} whose state changed. Use the getters to
     *               obtain the latest
     *               states.
     * @param events The {@link Events} that happened in this iteration, indicating
     *               which player
     *               states changed.
     */
     ...
     *
     * <p>
     * Note that the current {@link MediaItem} or playback position may change as a
     * result of a
     * timeline change. If playback can't continue smoothly because of this timeline
     * change, a
     * separate {@link #onPositionDiscontinuity(PositionInfo, PositionInfo, int)}
     * callback will be
     * triggered.
     *
     * <p>
     * {@link #onEvents(Player, Events)} will also be called to report this event
     * along with
     * other events that happen in the same {@link Looper} message queue iteration.
     *
     * @param timeline The latest timeline. Never null, but may be empty.
     * @param reason   The {@link TimelineChangeReason} responsible for this
     *                 timeline change.
     */
    ...
    /**
     * Called when playback transitions to a media item or starts repeating a media
     * item according
     ...
     * other events that happen in the same {@link Looper} message queue iteration.
     *
     * @param mediaItem The {@link MediaItem}. May be null if the playlist becomes
     *                  empty.
     ...
     * other events that happen in the same {@link Looper} message queue iteration.
     *
     * @param tracks The available tracks information. Never null, but may be of
     *               length zero.
     */
    ...
    /**
     * Called when the value returned from {@link #isCommandAvailable(int)} changes
     * for at least one
     * {@link Command}.
     *
     * <p>
     ...
     *
     * <p>
     * The current {@code playWhenReady} value may be re-reported if the
     * {@code reason} for this
     * value changes.
     *
     * <p>
     * {@link #onEvents(Player, Events)} will also be called to report this event
    ...
    /**
     * Called when the value returned from {@link #getPlaybackSuppressionReason()}
     * changes.
     *
     * <p>
     * {@link #onEvents(Player, Events)} will also be called to report this event
     ...
     * other events that happen in the same {@link Looper} message queue iteration.
     *
     * @param playbackSuppressionReason The current
     *                                  {@link PlaybackSuppressionReason}.
     */
     ...
     * other events that happen in the same {@link Looper} message queue iteration.
     *
     * @param shuffleModeEnabled Whether shuffling of {@linkplain MediaItem media
     *                           items} is enabled.
     */
     ...
     * {@link #STATE_IDLE}
     * immediately after this method is called. The player instance can still be
     * used, and {@link
     * #release()} must still be called on the player should it no longer be
     * required.
     *
     * <p>
     * {@link #onEvents(Player, Events)} will also be called to report this event
     ...
     *
     * <p>
     * Implementations of Player may pass an instance of a subclass of
     * {@link PlaybackException}
    ...
    /**
     * Called when the {@link PlaybackException} returned by
     * {@link #getPlayerError()} changes.
     *
     * <p>
     * {@link #onEvents(Player, Events)} will also be called to report this event
     * along with
    ...
    /**
     * @deprecated Use
     *             {@link #onPositionDiscontinuity(PositionInfo, PositionInfo, int)}
     *             instead.
     */
    @Deprecated
    @UnstableApi
     ...
     * position
     * jumps within the period currently being played, or when the playing period
     * has been skipped
     * or removed.
     *
     * <p>
     ...
     * @param reason      The {@link DiscontinuityReason} responsible for the
     *                    discontinuity.
     */
    ...
    /**
     * Called when the value of {@link #getPlaybackParameters()} changes. The
     * playback parameters
     * may change due to a call to
     * {@link #setPlaybackParameters(PlaybackParameters)}, or the player
     * itself may change them (for example, if audio playback switches to
     ...
     * other events that happen in the same {@link Looper} message queue iteration.
     *
     * @param seekBackIncrementMs The {@link #seekBack()} increment, in
     *                            milliseconds.
     */
     ...
     * other events that happen in the same {@link Looper} message queue iteration.
     *
     * @param seekForwardIncrementMs The {@link #seekForward()} increment, in
     *                               milliseconds.
     */
     ...
     * other events that happen in the same {@link Looper} message queue iteration.
     *
     * @param maxSeekToPreviousPositionMs The maximum position for which
     *                                    {@link #seekToPrevious()}
     *                                    seeks to the previous position, in
     *                                    milliseconds.
     */
     ...
     * other events that happen in the same {@link Looper} message queue iteration.
     *
     * @param skipSilenceEnabled Whether skipping silences in the audio stream is
     *                           enabled.
     */
    ...
    /**
     * Called when the value of {@link #getDeviceVolume()} or
     * {@link #isDeviceMuted()} changes.
     *
     * <p>
     * {@link #onEvents(Player, Events)} will also be called to report this event
     * along with
     * other events that happen in the same {@link Looper} message queue iteration.
     *
     * @param volume The new device volume, with 0 being silence and 1 being unity
     *               gain.
    ...
    /**
     * Called each time there's a change in the size of the surface onto which the
     * video is being
     * rendered.
     *
     * <p>
     ...
     * other events that happen in the same {@link Looper} message queue iteration.
     *
     * @param width  The surface width in pixels. May be {@link C#LENGTH_UNSET} if
     *               unknown, or 0 if
     *               the video is not rendered onto a surface.
     * @param height The surface height in pixels. May be {@link C#LENGTH_UNSET} if
     *               unknown, or 0 if
     *               the video is not rendered onto a surface.
     */
    ...
    /**
     * Called when a frame is rendered for the first time since setting the surface,
     * or since the
     ...
     *
     * <p>
     * Both this method and {@link #onCues(CueGroup)} are called when there is a
     * change in the
     ...
     *
     * <p>
     * Both this method and {@link #onCues(List)} are called when there is a change
     * in the cues.
  ...
  /**
   * Playback state. One of {@link #STATE_IDLE}, {@link #STATE_BUFFERING},
   * {@link #STATE_READY} or
   * {@link #STATE_ENDED}.
   */
  ...
  /**
   * The player is idle, meaning it holds only limited resources. The player must
   * be {@link
  ...
  /**
   * The player is not able to immediately play the media, but is doing work
   * toward being able to do
   * so. This state typically occurs when the player needs to buffer more data
   * before playback can
   * start.
   */
  ...
  /**
   * The player is able to immediately play from its current position. The player
   * will be playing if
   ...
   * {@linkplain #getPlaybackSuppressionReason()
   * suppressed} too long.
   */
  int PLAY_WHEN_READY_CHANGE_REASON_SUPPRESSED_TOO_LONG = 6;

  /**
   * Reason why playback is suppressed even though {@link #getPlayWhenReady()} is
   * {@code true}.
   *
   * <p>
   * One of:
  ...
  /**
   * @deprecated Use {@link #PLAYBACK_SUPPRESSION_REASON_UNSUITABLE_AUDIO_OUTPUT}
   *             instead.
   */
  @Deprecated
  ...
  /**
   * Playback is suppressed due to attempt to play on an unsuitable audio output
   * (e.g. attempt to
  ...
  /**
   * Repeat modes for playback. One of {@link #REPEAT_MODE_OFF},
   * {@link #REPEAT_MODE_ONE} or {@link
   * #REPEAT_MODE_ALL}.
   */
  // @Target list includes both 'default' targets and TYPE_USE, to ensure
  // backwards compatibility
  ...
  /**
   * Normal playback without repetition. "Previous" and "Next" actions move to the
   * previous and next
   * {@link MediaItem} respectively, and do nothing when there is no previous or
   * next {@link
  ...
  /**
   * Repeats the currently playing {@link MediaItem} infinitely during ongoing
   * playback. "Previous"
   * and "Next" actions behave as they do in {@link #REPEAT_MODE_OFF}, moving to
   * the previous and
   * next {@link MediaItem} respectively, and doing nothing when there is no
  ...
  /**
   * Repeats the entire timeline infinitely. "Previous" and "Next" actions behave
   * as they do in
   * {@link #REPEAT_MODE_OFF}, but with looping at the ends so that "Previous"
   * when playing the
   * first {@link MediaItem} will move to the last {@link MediaItem}, and "Next"
   * when playing the
  ...
  /**
   * Reasons for position discontinuities. One of
   * {@link #DISCONTINUITY_REASON_AUTO_TRANSITION},
   * {@link #DISCONTINUITY_REASON_SEEK},
   * {@link #DISCONTINUITY_REASON_SEEK_ADJUSTMENT}, {@link
   ...
   * The period index may
   * be the same as it was before the discontinuity in case the current period is
   * repeated.
   *
   * <p>
   * This reason also indicates an automatic transition from the content period to
   * an inserted ad
   * period or vice versa. Or a transition caused by another player (e.g. multiple
   * controllers can
  ...
  /**
   * Seek adjustment due to being unable to seek to the requested position or
   * because the seek was
  ...
  /**
   * Reasons for timeline changes. One of
   * {@link #TIMELINE_CHANGE_REASON_PLAYLIST_CHANGED} or {@link
  ...
  /**
   * Timeline changed as a result of a change of the playlist items or the order
   * of the items.
   */
  int TIMELINE_CHANGE_REASON_PLAYLIST_CHANGED = 0;

  /**
   * Timeline changed as a result of a source update (e.g. result of a dynamic
   * update by the played
   * media).
   *
   * <p>
   * This reason also indicates a change caused by another player (e.g. multiple
   * controllers can
  ...
  /**
   * Reasons for media item transitions. One of
   * {@link #MEDIA_ITEM_TRANSITION_REASON_REPEAT}, {@link
   * #MEDIA_ITEM_TRANSITION_REASON_AUTO},
   * {@link #MEDIA_ITEM_TRANSITION_REASON_SEEK} or {@link
   ...
   *
   * <p>
   * This reason also indicates a transition caused by another player (e.g.
  ...
  /**
   * The current media item has changed because of a change in the playlist. This
   * can either be if
   * the media item previously being played has been removed, or when the playlist
   * becomes non-empty
   * after being empty.
   */
  ...
  /**
   * {@link #getCurrentMediaItem()} changed or the player started repeating the
   * current item.
   */
   ...
   * {@link Listener#onPositionDiscontinuity(PositionInfo,
   * PositionInfo, int)}.
   */
  ...
  /**
   * A frame is rendered for the first time since setting the surface, or since
   * the renderer was
  ...
  /**
   * Commands that indicate which method calls are currently permitted on a
   * particular {@code
   * Player} instance.
   *
   * <p>
   * The currently available commands can be inspected with
   * {@link #getAvailableCommands()} and
   * {@link #isCommandAvailable(int)}.
   *
   * <p>
   * See the documentation of each command constant for the details of which
   * methods it permits
   * calling.
   *
   * <p>
   ...
   *
   * <p>
   * The {@link #prepare()} method must only be called if this command is
   * {@linkplain
   * #isCommandAvailable(int) available}.
   */
   ...
   *
   * <p>
   * The {@link #stop()} method must only be called if this command is {@linkplain
   * #isCommandAvailable(int) available}.
   */
  int COMMAND_STOP = 3;
   ...
   *
   * <p>
   * The {@link #seekToDefaultPosition()} method must only be called if this
   * command is
   ...
   *
   * <p>
   * The {@link #seekTo(long)} method must only be called if this command is
   * {@linkplain
   * #isCommandAvailable(int) available}.
   */
   ...
   *
   * <p>
   * The {@link #seekToPreviousMediaItem()} method must only be called if this
   * command is
   * {@linkplain #isCommandAvailable(int) available}.
   */
  ...
  /**
   * Command to seek to an earlier position in the current {@link MediaItem} or
   * the default position
   * of the previous {@link MediaItem}.
   *
   * <p>
   * The {@link #seekToPrevious()} method must only be called if this command is
   * {@linkplain
   * #isCommandAvailable(int) available}.
   */
   ...
   *
   * <p>
   * The {@link #seekToNextMediaItem()} method must only be called if this command
   * is {@linkplain
   * #isCommandAvailable(int) available}.
   */
  ...
  /**
   * Command to seek to a later position in the current {@link MediaItem} or the
   * default position of
   * the next {@link MediaItem}.
   *
   * <p>
   * The {@link #seekToNext()} method must only be called if this command is
   * {@linkplain
   * #isCommandAvailable(int) available}.
   */
   ...
   *
   * <p>
   * The {@link #seekBack()} method must only be called if this command is
   * {@linkplain
   * #isCommandAvailable(int) available}.
   */
   ...
   *
   * <p>
   * The {@link #seekForward()} method must only be called if this command is
   * {@linkplain
   * #isCommandAvailable(int) available}.
   */
   ...
   *
   * <p>
   * The {@link #setShuffleModeEnabled(boolean)} method must only be called if
   * this command is
   * {@linkplain #isCommandAvailable(int) available}.
   */
   ...
   *
   * <p>
   * The {@link #setRepeatMode(int)} method must only be called if this command is
   * {@linkplain
   * #isCommandAvailable(int) available}.
   */
   ...
   *
   * <p>
   * The {@link #setPlaylistMetadata(MediaMetadata)} method must only be called if
   * this command
   ...
   *
   * <p>
   * The {@link #getAudioAttributes()} method must only be called if this command
   * is {@linkplain
   * #isCommandAvailable(int) available}.
   */
   ...
   *
   * <p>
   * The {@link #getVolume()} method must only be called if this command is
   * {@linkplain
   * #isCommandAvailable(int) available}.
   */
   ...
   *
   * <p>
   * The {@link #setVolume(float)} method must only be called if this command is
   * {@linkplain
   * #isCommandAvailable(int) available}.
   */
   ...
   *
   * <p>
   * The {@link #setDeviceVolume(int, int)} method must only be called if this
   * command is
   * {@linkplain #isCommandAvailable(int) available}.
   */
  ...
  /**
   * Command to increase and decrease the device volume and mute it with volume
   * flags.
   *
   * <p>
   * The following methods must only be called if this command is {@linkplain
   ...
   *
   * <p>
   * The {@link #setAudioAttributes(AudioAttributes, boolean)} method must only be
   * called if this
   ...
   *
   * <p>
   * The {@link #getCurrentCues()} method must only be called if this command is
   * {@linkplain
   * #isCommandAvailable(int) available}.
   */
   ...
   *
   * <p>
   * The {@link #setTrackSelectionParameters(TrackSelectionParameters)} method
   * must only be
   ...
   *
   * <p>
   * The {@link #getCurrentTracks()} method must only be called if this command is
   * {@linkplain
   * #isCommandAvailable(int) available}.
   */
   ...
   *
   * <p>
   * The {@link #getSubtitleOffsetMs()} method must only be called if this command
   * is
  ...
  /**
   * Command to set the subtitle offset applied at runtime. Positive values delay
   * subtitles;
   * negative values show them earlier.
   *
   * <p>
   * The {@link #setSubtitleOffsetMs(long)} method must only be called if this
   * command is
   * {@linkplain #getAvailableCommands() available}.
   */
  @UnstableApi
   ...
   *
   * <p>
   * The {@link #release()} method must only be called if this command is
   * {@linkplain
   * #isCommandAvailable(int) available}.
   */
  ...
  /**
   * Returns the {@link Looper} associated with the application thread that's used
   * to access the
  ...
  /**
   * Unregister a listener registered through {@link #addListener(Listener)}. The
   * listener will no
  ...
  /**
   * Clears the playlist, adds the specified {@linkplain MediaItem media items}
   * and resets the
   * position to the default position.
   *
   * <p>
   * To replace a span of media items (possibly seamlessly) without clearing the
   * playlist, use
   * {@link #replaceMediaItems}.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_CHANGE_MEDIA_ITEMS} is
   * {@linkplain
  ...
  /**
   * Clears the playlist and adds the specified {@linkplain MediaItem media
   * items}.
   *
   * <p>
   * To replace a span of media items (possibly seamlessly) without clearing the
   ...
   *
   * @param mediaItems    The new {@linkplain MediaItem media items}.
   * @param resetPosition Whether the playback position should be reset to the
   *                      default position in
   *                      the first {@link Timeline.Window}. If false, playback
   *                      will start from the position defined
   *                      by {@link #getCurrentMediaItemIndex()} and
   ...
   *
   * @param mediaItems      The new {@linkplain MediaItem media items}.
   * @param startIndex      The {@link MediaItem} index to start playback from. If
   *                        {@link C#INDEX_UNSET}
   *                        is passed, the current position is not reset.
   * @param startPositionMs The position in milliseconds to start playback from.
   *                        If {@link
   *                        C#TIME_UNSET} is passed, the default position of the
   *                        given {@link MediaItem} is used. In
   *                        any case, if {@code startIndex} is set to
   *                        {@link C#INDEX_UNSET}, this parameter is ignored
   *                        and the position is not reset at all.
   * @throws IllegalSeekPositionException If the provided {@code startIndex} is
   *                                      not within the
  ...
  /**
   * Clears the playlist, adds the specified {@link MediaItem} and resets the
   * position to the
   * default position.
   *
   * <p>
   * To replace a media item (possibly seamlessly) without clearing the playlist,
   * use {@link
   * #replaceMediaItem}.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_SET_MEDIA_ITEM} is
   * {@linkplain
   * #getAvailableCommands() available}.
   *
   ...
   * @param resetPosition Whether the playback position should be reset to the
   *                      default position. If
   *                      false, playback will start from the position defined by
   ...
   * #getAvailableCommands() available}.
   *
   * @param index     The index at which to add the media item. If the index is
   *                  larger than the size of
   *                  the playlist, the media item is added to the end of the
   *                  playlist.
   * @param mediaItem The {@link MediaItem} to add.
   */
   ...
   * #getAvailableCommands() available}.
   *
   * @param index      The index at which to add the media items. If the index is
   *                   larger than the size of
   *                   the playlist, the media items are added to the end of the
   *                   playlist.
   * @param mediaItems The {@linkplain MediaItem media items} to add.
   */
   ...
   * #getAvailableCommands() available}.
   *
   * @param currentIndex The current index of the media item to move. If the index
   *                     is larger than
   *                     the size of the playlist, the request is ignored.
   * @param newIndex     The new index of the media item. If the new index is
   *                     larger than the size of
   *                     the playlist the item is moved to the end of the
   *                     playlist.
   */
   ...
   * #getAvailableCommands() available}.
   *
   * @param fromIndex The start of the range to move. If the index is larger than
   *                  the size of the
   *                  playlist, the request is ignored.
   * @param toIndex   The first item not to be included in the range (exclusive).
   *                  If the index is
   *                  larger than the size of the playlist, items up to the end of
   *                  the playlist are moved.
   * @param newIndex  The new index of the first media item of the range. If the
   *                  new index is larger
   *                  than the size of the remaining playlist after removing the
   *                  range, the range is moved to the
   *                  end of the playlist.
   */
   ...
   *
   * <p>
   * Implementations of this method may attempt to seamlessly continue playback if
   * the currently
   * playing media item is replaced with a compatible one (e.g. same URL, only
   * metadata has
   * changed).
   *
   * <p>
   ...
   * #getAvailableCommands() available}.
   *
   * @param index     The index at which to replace the media item. If the index
   ...
   *
   * <p>
   * Note that it is possible to replace a range with an arbitrary number of new
   * items, so that
   * the number of removed items defined by {@code fromIndex} and {@code toIndex}
   * does not have to
   * match the number of added items defined by {@code mediaItems}. As result, it
   * may also change
   * the index of subsequent items not touched by this operation.
   *
   * @param fromIndex  The start of the range. If the index is larger than the
   *                   size of the playlist,
   *                   the request is ignored.
   * @param toIndex    The first item not to be included in the range (exclusive).
   *                   If the index is
   *                   larger than the size of the playlist, items up to the end
   *                   of the playlist are replaced.
   * @param mediaItems The {@linkplain MediaItem media items} to replace the range
   *                   with.
   */
   ...
   * #getAvailableCommands() available}.
   *
   * @param index The index at which to remove the media item. If the index is
   ...
   * #getAvailableCommands() available}.
   *
   * @param fromIndex The index at which to start removing media items. If the
   *                  index is larger than
   *                  the size of the playlist, the request is ignored.
   * @param toIndex   The index of the first item to be kept (exclusive). If the
   *                  index is larger than
   *                  the size of the playlist, media items up to the end of the
   ...
   *
   * <p>
   * The returned {@link Commands} are not updated when available commands change.
   * Use {@link
   * Listener#onAvailableCommandsChanged(Commands)} to get an update when the
   * available commands
   * change.
   *
   ...
   *
   * <p>
   * This will move the player out of {@link #STATE_IDLE idle state} and the
   * player will start
   ...
   * {@link #getPlayWhenReady()} is {@code
   * true}, or {@link #PLAYBACK_SUPPRESSION_REASON_NONE} if playback is not
   * suppressed.
   *
  ...
  /**
   * Returns whether the player is playing, i.e. {@link #getCurrentPosition()} is
   * advancing.
   *
   * <p>
   ...
   *
   * <ul>
   * <li>The {@link #getPlaybackState() playback state} is not {@link #STATE_READY
   * ready}.
   * <li>There is no {@link #getPlayWhenReady() intention to play}.
   * <li>Playback is {@link #getPlaybackSuppressionReason() suppressed for other
   * reasons}.
   * </ul>
   *
  ...
  /**
   * Returns the error that caused playback to fail. This is the same error that
   * will have been
   * reported via {@link Listener#onPlayerError(PlaybackException)} at the time of
   * failure. It can
   * be queried using this method until the player is re-prepared.
   *
   * <p>
   * Note that this method will always return {@code null} if
   * {@link #getPlaybackState()} is not
   * {@link #STATE_IDLE}.
   *
  ...
  /**
   * Resumes playback as soon as {@link #getPlaybackState()} ==
   * {@link #STATE_READY}. Equivalent to
  ...
  /**
   * Pauses playback. Equivalent to {@link #setPlayWhenReady(boolean)
   ...
   *
   * <p>
   * If the player is already in the ready state then this method pauses and
   * resumes playback.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_PLAY_PAUSE} is {@linkplain
   ...
   *
   * <p>
   * This method must only be called if {@link #COMMAND_SET_REPEAT_MODE} is
   * {@linkplain
   * #getAvailableCommands() available}.
   *
   ...
   *
   * <p>
   * This method must only be called if {@link #COMMAND_SET_SHUFFLE_MODE} is
   * {@linkplain
   * #getAvailableCommands() available}.
   *
  ...
  /**
   * Seeks to the default position associated with the current {@link MediaItem}.
   * The position can
   * depend on the type of media being played. For live streams it will typically
   * be the live edge.
   * For other streams it will typically be the start.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_SEEK_TO_DEFAULT_POSITION}
   * is {@linkplain
   * #getAvailableCommands() available}.
   */
   ...
   * {@link MediaItem}. The position can
   * depend on the type of media being played. For live streams it will typically
   * be the live edge.
   * For other streams it will typically be the start.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_SEEK_TO_MEDIA_ITEM} is
   * {@linkplain
   * #getAvailableCommands() available}.
   *
   * @param mediaItemIndex The index of the {@link MediaItem} whose associated
   *                       default position
   *                       should be seeked to. If the index is larger than the
   *                       size of the playlist, the request is
   *                       ignored.
   */
   ...
   * {@link #COMMAND_SEEK_IN_CURRENT_MEDIA_ITEM} is
   * {@linkplain #getAvailableCommands() available}.
   *
   * @param positionMs The seek position in the current {@link MediaItem}, or
   *                   {@link C#TIME_UNSET}
   ...
   * #getAvailableCommands() available}.
   *
   * @param mediaItemIndex The index of the {@link MediaItem}. If the index is
   *                       larger than the size
   *                       of the playlist, the request is ignored.
   * @param positionMs     The seek position in the specified {@link MediaItem},
   *                       or {@link C#TIME_UNSET}
   *                       to seek to the media item's default position.
   */
  ...
  /**
   * Seeks back in the current {@link MediaItem} by
   * {@link #getSeekBackIncrement()} milliseconds.
   *
   * <p>
  ...
  /**
   * Seeks forward in the current {@link MediaItem} by
   * {@link #getSeekForwardIncrement()}
   * milliseconds.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_SEEK_FORWARD} is
   * {@linkplain
   * #getAvailableCommands() available}.
   */
  void seekForward();
  ...
  /**
   * Returns whether a previous media item exists, which may depend on the current
   * repeat mode and
   ...
   * the same as when
   * the current repeat mode is {@link #REPEAT_MODE_OFF}. See
   * {@link #REPEAT_MODE_ONE} for more
   * details.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_GET_TIMELINE} is
   * {@linkplain
   * #getAvailableCommands() available}.
   */
  ...
  /**
   * Seeks to the default position of the previous {@link MediaItem}, which may
   * depend on the
   ...
   * <p>
   * This method must only be called if
   * {@link #COMMAND_SEEK_TO_PREVIOUS_MEDIA_ITEM} is
   * {@linkplain #getAvailableCommands() available}.
   */
  void seekToPreviousMediaItem();

  /**
   * Returns the maximum position for which {@link #seekToPrevious()} seeks to the
   * previous {@link
  ...
  /**
   * Seeks to an earlier position in the current or previous {@link MediaItem} (if
   * available). More
   * precisely:
   *
   * <ul>
   * <li>If the timeline is empty or seeking is not possible, does nothing.
   * <li>Otherwise, if the current {@link MediaItem} is
   * {@linkplain #isCurrentMediaItemLive()
   * live} and {@linkplain #isCurrentMediaItemSeekable() unseekable}, then:
   * <ul>
   * <li>If {@linkplain #hasPreviousMediaItem() a previous media item exists},
   * seeks to the
   * default position of the previous media item.
   * <li>Otherwise, does nothing.
   * </ul>
   * <li>Otherwise, if {@linkplain #hasPreviousMediaItem() a previous media item
   * exists} and the
   * {@linkplain #getCurrentPosition() current position} is less than {@link
   * #getMaxSeekToPreviousPosition()}, seeks to the default position of the
   * previous {@link
   * MediaItem}.
   ...
   *
   * <p>
   * This method must only be called if {@link #COMMAND_SEEK_TO_PREVIOUS} is
   * {@linkplain
   * #getAvailableCommands() available}.
   */
  void seekToPrevious();
  ...
  /**
   * Returns whether a next {@link MediaItem} exists, which may depend on the
   * current repeat mode
  ...
  /**
   * Seeks to the default position of the next {@link MediaItem}, which may depend
   * on the current
   * repeat mode and whether shuffle mode is enabled. Does nothing if
   * {@link #hasNextMediaItem()} is
   * {@code false}.
   *
   * <p>
   * Note: When the repeat mode is {@link #REPEAT_MODE_ONE}, this method behaves
   ...
   *
   * <p>
   * This method must only be called if {@link #COMMAND_SEEK_TO_NEXT_MEDIA_ITEM}
   * is {@linkplain
   * #getAvailableCommands() available}.
   */
  ...
  /**
   * Seeks to a later position in the current or next {@link MediaItem} (if
   * available). More
   * precisely:
   *
   * <ul>
   * <li>If the timeline is empty or seeking is not possible, does nothing.
   * <li>Otherwise, if {@linkplain #hasNextMediaItem() a next media item exists},
   * seeks to the
   * default position of the next {@link MediaItem}.
   * <li>Otherwise, if the current {@link MediaItem} is
   * {@linkplain #isCurrentMediaItemLive()
   * live} and has not ended, seeks to the live edge of the current
   * {@link MediaItem}.
   * <li>Otherwise, does nothing.
   * </ul>
   *
   * <p>
   * This method must only be called if {@link #COMMAND_SEEK_TO_NEXT} is
   * {@linkplain
   * #getAvailableCommands() available}.
   */
  void seekToNext();
  ...
  /**
   * Attempts to set the playback parameters. Passing
   * {@link PlaybackParameters#DEFAULT} resets the
   ...
   * Listener#onPlaybackParametersChanged(PlaybackParameters)} will be called
   * whenever the currently
   * active playback parameters change.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_SET_SPEED_AND_PITCH} is
   * {@linkplain
   * #getAvailableCommands() available}.
   *
   * @param playbackParameters The playback parameters.
   ...
   * #getAvailableCommands() available}.
   *
   * @param speed The linear factor by which playback will be sped up. Must be
   *              higher than 0. 1 is
  ...
  /**
   * Stops playback without resetting the playlist. Use {@link #pause()} rather
   * than this method if
   ...
   * The player
   * instance can still be used by calling {@link #prepare()} again, and
   * {@link #release()} must
   * still be called on the player if it's no longer required.
   *
   * <p>
   * Calling this method does not clear the playlist, reset the playback position
   * or the playback
   * error.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_STOP} is {@linkplain
   * #getAvailableCommands() available}.
   */
  void stop();
   ...
   *
   * <p>
   * This method must only be called if {@link #COMMAND_RELEASE} is {@linkplain
   * #getAvailableCommands() available}.
   */
  void release();
   ...
   *
   * <p>
   * This method must only be called if {@link #COMMAND_GET_TRACKS} is {@linkplain
   * #getAvailableCommands() available}.
   *
   ...
   *
   * <p>
   * Use {@link #getTrackSelectionParameters()} to retrieve the current
   * parameters. For example,
   * the following snippet restricts video to SD whilst keep other track selection
   * parameters
   * unchanged:
   *
   * <pre>{@code
   ...
   * <p>
   * This method must only be called if
   * {@link #COMMAND_SET_TRACK_SELECTION_PARAMETERS} is
   * {@linkplain #getAvailableCommands() available}.
   */
  void setTrackSelectionParameters(TrackSelectionParameters parameters);

  /**
   * Returns the current combined {@link MediaMetadata}, or
   * {@link MediaMetadata#EMPTY} if not
   * supported.
   *
   * <p>
   * This {@link MediaMetadata} is a combination of the
   * {@link MediaItem#mediaMetadata MediaItem
   * metadata}, the static metadata in the media's {@link Format#metadata Format},
   * and any timed
   * metadata that has been parsed from the media and output via {@link
   * Listener#onMetadata(Metadata)}. If a field is populated in the
   * {@link MediaItem#mediaMetadata},
   * it will be prioritised above the same field coming from static or timed
   * metadata.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_GET_METADATA} is
   * {@linkplain
   * #getAvailableCommands() available}.
   *
   ...
   * #setPlaylistMetadata(MediaMetadata)}, or {@link MediaMetadata#EMPTY} if not
   * supported.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_GET_METADATA} is
   ...
   *
   * <p>
   * This method must only be called if {@link #COMMAND_SET_PLAYLIST_METADATA} is
   * {@linkplain
   * #getAvailableCommands() available}.
   */
  ...
  /**
   * Returns the current manifest. The type depends on the type of media being
   * played. May be null.
   */
  @UnstableApi
  @Nullable
  ...
  /**
   * Returns the index of the current {@link MediaItem} in the
   * {@link #getCurrentTimeline()
   * timeline}, or the prospective index if the {@link #getCurrentTimeline()
   * current timeline} is
   * empty.
   *
   * <p>
   ...
   * #seekToNextMediaItem()} is called, which may depend on the current repeat
   * mode and whether
   * shuffle mode is enabled. Returns {@link C#INDEX_UNSET} if
   * {@link #hasNextMediaItem()} is {@code
   * false}.
   *
   * <p>
   * Note: When the repeat mode is {@link #REPEAT_MODE_ONE}, this method behaves
  ...
  /**
   * Returns the index of the {@link MediaItem} that will be played if {@link
   * #seekToPreviousMediaItem()} is called, which may depend on the current repeat
   * mode and whether
   * shuffle mode is enabled. Returns {@link C#INDEX_UNSET} if
   * {@link #hasPreviousMediaItem()} is
   * {@code false}.
   *
   * <p>
   * Note: When the repeat mode is {@link #REPEAT_MODE_ONE}, this method behaves
  ...
  /**
   * Returns the currently playing {@link MediaItem}. May be null if the timeline
   * is empty.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_GET_CURRENT_MEDIA_ITEM} is
   * {@linkplain
   * #getAvailableCommands() available}.
   *
  ...
  /**
   * Returns the playback position in the current content or ad, in milliseconds,
   * or the prospective
   * position in milliseconds if the {@link #getCurrentTimeline() current
   * timeline} is empty.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_GET_CURRENT_MEDIA_ITEM} is
  ...
  /**
   * Returns an estimate of the position in the current content or ad up to which
   * data is buffered,
   * in milliseconds.
   *
   * <p>
  ...
  /**
   * Returns an estimate of the percentage in the current content or ad up to
   * which data is
  ...
  /**
   * Returns an estimate of the total buffered duration from the current position,
   * in milliseconds.
   * This includes pre-buffered data for subsequent ads and {@linkplain MediaItem
   * media items}.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_GET_CURRENT_MEDIA_ITEM} is
  ...
  /**
   * Returns whether the current {@link MediaItem} is dynamic (may change when the
   * {@link Timeline}
  ...
  /**
   * Returns whether the current {@link MediaItem} is live, or {@code false} if
   * the {@link Timeline}
   * is empty.
   *
   * <p>
  ...
  /**
   * Returns the offset of the current playback position from the live edge in
   * milliseconds, or
   * {@link C#TIME_UNSET} if the current {@link MediaItem}
   * {@linkplain #isCurrentMediaItemLive()
   * isn't live} or the offset is unknown.
   *
   * <p>
   * The offset is calculated as {@code currentTime - playbackPosition}, so should
   * usually be
   * positive.
   *
   * <p>
   * Note that this offset may rely on an accurate local time, so this method may
   * return an
   * incorrect value if the difference between system clock and server clock is
   * unknown.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_GET_CURRENT_MEDIA_ITEM} is
  ...
  /**
   * Returns whether the current {@link MediaItem} is seekable, or {@code false}
   * if the {@link
   * Timeline} is empty.
   *
   * <p>
  ...
  /**
   * If {@link #isPlayingAd()} returns true, returns the index of the ad group in
   * the period
  ...
  /**
   * If {@link #isPlayingAd()} returns true, returns the index of the ad in its ad
   * group. Returns
   * {@link C#INDEX_UNSET} otherwise.
   *
   * <p>
  ...
  /**
   * If {@link #isPlayingAd()} returns {@code true}, returns the duration of the
   * current content in
   * milliseconds, or {@link C#TIME_UNSET} if the duration is not known. If there
   * is no ad playing,
  ...
  /**
   * If {@link #isPlayingAd()} returns {@code true}, returns the content position
   * that will be
   * played once all ads in the ad group have finished playing, in milliseconds.
   * If there is no ad
   * playing, the returned position is the same as that returned by
   * {@link #getCurrentPosition()}.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_GET_CURRENT_MEDIA_ITEM} is
  ...
  /**
   * If {@link #isPlayingAd()} returns {@code true}, returns an estimate of the
   * content position in
   * the current content up to which data is buffered, in milliseconds. If there
   * is no ad playing,
   * the returned position is the same as that returned by
   ...
   *
   * <p>
   * This method must only be called if {@link #COMMAND_GET_AUDIO_ATTRIBUTES} is
   * {@linkplain
   * #getAvailableCommands() available}.
   *
  ...
  /**
   * Returns the audio session identifier, or {@link C#AUDIO_SESSION_ID_UNSET} if
   * not set.
   *
  ...
  /**
   * Sets the audio volume, valid values are between 0 (silence) and 1 (unity
   * gain, signal
   * unchanged), inclusive.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_SET_VOLUME} is {@linkplain
   * #getAvailableCommands() available}.
   *
  ...
  /**
   * Returns the audio volume, with 0 being silence and 1 being unity gain (signal
   * unchanged).
   *
   * <p>
   * This method must only be called if {@link #COMMAND_GET_VOLUME} is {@linkplain
   * #getAvailableCommands() available}.
   *
  ...
  /**
   * Sets the subtitle offset in milliseconds applied at runtime. Positive values
   * delay subtitles;
   * negative values show them earlier. This is applied in addition to any offset
   * set on the
   * {@link MediaItem.SubtitleConfiguration}.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_SET_SUBTITLE_OFFSET} is
   * {@linkplain
   * #getAvailableCommands() available}.
   *
   ...
   *
   * <p>
   * This method must only be called if {@link #COMMAND_GET_SUBTITLE_OFFSET} is
   * {@linkplain
   * #getAvailableCommands() available}.
   *
  ...
  /**
   * If the audio volume is 0, sets the audio volume to a non-zero value decided
   * by the Player to be
  ...
  /**
   * Clears any {@link Surface}, {@link SurfaceHolder}, {@link SurfaceView} or
   * {@link TextureView}
   * currently set on the player.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_SET_VIDEO_SURFACE} is
   * {@linkplain
   * #getAvailableCommands() available}.
   */
  ...
  /**
   * Clears the {@link Surface} onto which video is being rendered if it matches
   * the one passed.
   * Else does nothing.
   *
   * <p>
  ...
  /**
   * Sets the {@link Surface} onto which video will be rendered. The caller is
   * responsible for
   ...
   * {@link
   * SurfaceHolder} then it's recommended to use
   * {@link #setVideoSurfaceView(SurfaceView)}, {@link
   * #setVideoTextureView(TextureView)} or
   * {@link #setVideoSurfaceHolder(SurfaceHolder)} rather than
   * this method, since passing the holder allows the player to track the
  ...
  /**
   * Sets the {@link SurfaceHolder} that holds the {@link Surface} onto which
   * video will be
   * rendered. The player will track the lifecycle of the surface automatically.
   *
   * <p>
   * The thread that calls the {@link SurfaceHolder.Callback} methods must be the
   * thread
  ...
  /**
   * Clears the {@link SurfaceHolder} that holds the {@link Surface} onto which
   * video is being
  ...
  /**
   * Sets the {@link SurfaceView} onto which video will be rendered. The player
   * will track the
  ...
  /**
   * Clears the {@link SurfaceView} onto which video is being rendered if it
  ...
  /**
   * Sets the {@link TextureView} onto which video will be rendered. The player
   * will track the
   * lifecycle of the surface automatically.
   *
   * <p>
   * Consider using {@link SurfaceView} via {@link #setVideoSurfaceView} instead
   * of {@link
   * TextureView}. {@link SurfaceView} generally causes lower battery consumption,
   * and has better
   * handling for HDR and secure content. See <a
   * href=
   * "https://developer.android.com/guide/topics/media/ui/playerview#surfacetype">Choosing
   * a
  ...
  /**
   * Clears the {@link TextureView} onto which video is being rendered if it
   * matches the one passed.
   * Else does nothing.
   *
   * <p>
   ...
   * Tracks#isTypeSupported(int) no supported video track} or its size has not
   * been determined yet.
   *
   ...
   *
   * <p>
   * This method must only be called if {@link #COMMAND_GET_TEXT} is {@linkplain
   * #getAvailableCommands() available}.
   *
   ...
   *
   * <p>
   * For devices with {@link DeviceInfo#PLAYBACK_TYPE_LOCAL local playback}, the
   * volume returned
   * by this method varies according to the current {@link C.StreamType stream
   * type}. The stream
   * type is determined by {@link AudioAttributes#getStreamType()}.
   *
   * <p>
   * For devices with {@link DeviceInfo#PLAYBACK_TYPE_REMOTE remote playback}, the
   * volume of the
   * remote device is returned.
   *
   * <p>
   * Note that this method returns the volume of the device. To check the current
   * stream volume,
   * use {@link #getVolume()}.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_GET_DEVICE_VOLUME} is
   * {@linkplain
   * #getAvailableCommands() available}.
   *
   ...
   *
   * <p>
   * Note that this method returns the mute state of the device. To check if the
   * current stream
   ...
   *
   * <p>
   * Note that this method affects the device volume. To change the volume of the
   * current stream
   * only, use {@link #setVolume}.
   *
   * <p>
   * This method must only be called if
   * {@link #COMMAND_SET_DEVICE_VOLUME_WITH_FLAGS} is
   * {@linkplain #getAvailableCommands() available}.
   *
   * @param volume The volume to set.
   * @param flags  Either 0 or a bitwise combination of one or more
   *               {@link C.VolumeFlags}.
   */
   ...
   * <p>
   * This method must only be called if
   * {@link #COMMAND_ADJUST_DEVICE_VOLUME_WITH_FLAGS} is
   * {@linkplain #getAvailableCommands() available}.
   *
   * @param flags Either 0 or a bitwise combination of one or more
   *              {@link C.VolumeFlags}.
   ...
   *
   * <p>
   * Note that this method affects the device volume. To mute just the current
   * stream, use {@code
   * setVolume(0)} instead.
   *
   * <p>
   ...
   * If not set, the
   * default audio attributes will be used. They are suitable for general media
   * playback.
   *
   * <p>
   * Setting the audio attributes during playback may introduce a short gap in
   * audio output as
   * the audio track is recreated. A new audio session id will also be generated.
   *
   * <p>
   * If tunneling is enabled by the track selector, the specified audio attributes
   * will be
   * ignored, but they will take effect if audio is later played without
   * tunneling.
   *
   * <p>
   * If audio focus should be handled, the {@link AudioAttributes#usage} must be
   * {@link
   * C#USAGE_MEDIA} or {@link C#USAGE_GAME}. Other usages will throw an {@link
   * IllegalArgumentException}.
   *
   * <p>
   * This method must only be called if {@link #COMMAND_SET_AUDIO_ATTRIBUTES} is
   * {@linkplain
   * #getAvailableCommands() available}.
   *
   * @param audioAttributes  The attributes to use for audio playback.
   * @param handleAudioFocus True if the player should handle audio focus, false
   *                         otherwise.
   */
